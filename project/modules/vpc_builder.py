...
class VPCBuilder:
    ...
    def destroy(self, vpc_id: str = None) -> bool:
        ...
        if not target_id:
            print(
                f"    [AWS SKIPPED] No live VPC found matching context '{self.name}' to drop."
            )
            return True

        try:
            print(f"    [AWS API] Terminating VPC context {target_id}...")
            self.ec2.delete_vpc(VpcId=target_id)
            return True
        except ClientError as e:
            print(f"    [AWS ERROR] Failed to drop VPC {target_id}: {e}")
            return False
