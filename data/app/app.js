...
window.sanitizeId = function(str) {
    ...
    return String(str).replace(/[^a-zA-Z0-9_]/g, '_').replace(/_{2,}/g, '_');
};
...
function renderTable(data, predefinedColumns) {
    ...
    if (!columns || columns.length === 0) {
        const keys = new Set();
        data.forEach(item => {
            if(item) Object.keys(item).forEach(k => k !== '__typename' && keys.add(k));
        });
        columns = Array.from(keys);
    }

    let thead = columns.map(c => `<th class="px-6 py-3 text-left text-xs font-bold text-slate-600 uppercase tracking-wider bg-slate-50 sticky top-0 shadow-sm border-b border-slate-200">${c.replace(/_/g, ' ')}</th>`).join('');
...
function setupDiagramPanZoom() {
    ...
    const stopDrag = () => { isDragging = false; el.classList.remove('dragging'); };
...
function renderIdentityTable() {
    ...
    if (identityHeaders.length === 0) {
        thead.innerHTML = '';
        tbody.innerHTML = '<tr><td class="p-4 text-center text-slate-500">No data</td></tr>';
        return;
    }
...
window.loadSettingsView = async function() {
    ...
    container.innerHTML = `
        <div class="p-8 overflow-y-auto flex-1 bg-white">
            <div class="mb-6">
                <h4 class="text-sm font-semibold text-slate-700 mb-3 border-b pb-2">Module Parameters</h4>
            ...
            <div class="mb-6">
                <div class="flex justify-between items-center mb-3 border-b pb-2">
                    <h4 class="text-sm font-semibold text-slate-700">Identities (identity.csv)</h4>
    ...
    try {
        ...
        if (pRes.ok) {
            ...
            for (const [key, paramData] of Object.entries(fetchedParams)) {
                ...
                let inputHtml = '';
                if (allowed.length > 0) {
                    let options = allowed.map(opt => `<option value="${esc(opt)}" ${opt === val ? 'selected' : ''}>${esc(opt)}</option>`).join('');
